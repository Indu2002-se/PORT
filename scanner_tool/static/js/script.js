// Global variables
    ...
    async function exportResults(format) {
        ...
        try {
            ...
            const response = await fetch(`/api/export/${format}?scan_id=${scanId}`);
            
            // Check if the response is OK
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Export API error:', errorText);
                showToast(`Export failed: Server returned ${response.status}`, 'error');
                return;
            }
            ...
            if (result.status === 'success') {
                ...
                showToast(`Export successful! Downloading ${result.filename}`, 'success');
                
                // Record this activity in the logs
                addLogEntry(`Exported results to ${format.toUpperCase()} format: ${result.filename}`, 'info');
            } else {
                console.error('Export result error:', result.message);
                showToast(`Export failed: ${result.message}`, 'error');
                addLogEntry(`Export failed: ${result.message}`, 'error');
            }
        } catch (error) {
            console.error('Export error:', error);
            showToast('Export failed: An unexpected error occurred.', 'error');
